...
def upgrade() -> None:
    ...
    agent_configs_columns = [
        'role_name', 'display_name', 'description', 'system_prompt_key',
        'model_provider', 'model_name', 'temperature', 'max_tokens', 'top_p',
        'capabilities', 'tools', 'autonomy_profiles', 'done_when',
    ...
    for config in default_configs:
        ...
        op.execute(
            f"INSERT INTO agent_configs ({', '.join(agent_configs_columns)}) "
            f"VALUES ({values})"
        )
    ...
    for policy in default_policies:
        ...
        op.execute(
            f"INSERT INTO gm_policies ({', '.join(gm_policies_columns)}) "
            f"VALUES ({values})"
        )
