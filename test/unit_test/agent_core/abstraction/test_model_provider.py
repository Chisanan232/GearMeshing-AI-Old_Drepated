...
class TestModelProviderAbstraction:
    ...
    def test_create_fallback_model_default_implementation(self):
        ...
        primary_config = ModelConfig(provider="openai", model="gpt-4o", temperature=0.7, max_tokens=1000)
        fallback_config = ModelConfig(provider="anthropic", model="claude-3-5-sonnet", temperature=0.5, max_tokens=2000)
    ...
    def test_create_fallback_model_exact_implementation_behavior(self):
        ...
        primary_config = ModelConfig(provider="openai", model="gpt-4o", temperature=0.8, max_tokens=1500, top_p=0.95)
        fallback_config = ModelConfig(
            provider="anthropic", model="claude-3-5-sonnet", temperature=0.6, max_tokens=2500, top_p=0.85
        )
    ...
    def test_get_provider_from_model_name_with_various_unknown_models(self):
        ...
        with patch("gearmeshing_ai.agent_core.abstraction.api_key_validator.APIKeyValidator") as mock_validator_class:
            ...
            unknown_models = ["llama-3-70b", "mistral-7b", "phi-3-medium", "custom-model-v1", "unknown-ai-model"]
...
class TestFallbackModelInstance:
    ...
    async def test_generate_primary_succeeds(self):
        ...
        expected_response = ModelResponse(
            content="Primary response", finish_reason="stop", usage={"prompt_tokens": 10, "completion_tokens": 20}
        )
        ...
        primary_model.generate.assert_called_once_with("test prompt", max_tokens=100, temperature=0.7)
    ...
    async def test_generate_primary_fails_fallback_succeeds(self):
        ...
        expected_response = ModelResponse(
            content="Fallback response", finish_reason="stop", usage={"prompt_tokens": 15, "completion_tokens": 25}
        )
        ...
        primary_model.generate.assert_called_once_with("test prompt", max_tokens=100, temperature=0.7)
        fallback_model.generate.assert_called_once_with("test prompt", max_tokens=100, temperature=0.7)
    ...
    async def test_generate_structured_primary_succeeds(self):
        ...
        result = await fallback_instance.generate_structured("test prompt", dict, max_tokens=100, temperature=0.7)

        # Verify primary was called and fallback was not
        primary_model.generate_structured.assert_called_once_with("test prompt", dict, max_tokens=100, temperature=0.7)
    ...
    async def test_generate_structured_primary_fails_fallback_succeeds(self):
        ...
        # Verify both models were called
        primary_model.generate_structured.assert_called_once_with("test prompt", dict, max_tokens=100, temperature=0.7)
        fallback_model.generate_structured.assert_called_once_with("test prompt", dict, max_tokens=100, temperature=0.7)
    ...
    async def test_generate_with_different_parameters(self):
        ...
        expected_response = ModelResponse(content="Response", finish_reason="stop", usage={})
        primary_model.generate = AsyncMock(return_value=expected_response)
    ...
    async def test_generate_structured_with_different_schemas(self):
        ...
        for schema in schemas:
            ...
            primary_model.generate_structured.assert_called_once_with(
                "test prompt", schema, max_tokens=None, temperature=None
            )
...
class TestModelProviderIntegration:
    ...
    def test_model_config_validation(self):
        ...
        # Test valid config
        valid_config = ModelConfig(provider="openai", model="gpt-4o", temperature=0.7, max_tokens=1000, top_p=0.9)
    ...
    async def test_fallback_model_error_logging_format(self):
        ...
        fallback_model.generate = AsyncMock(
            return_value=ModelResponse(content="Fallback response", finish_reason="stop", usage={})
        )
