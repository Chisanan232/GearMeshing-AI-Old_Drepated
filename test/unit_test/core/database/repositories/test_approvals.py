...
class TestApprovalRepository:
    ...
    async def test_list_with_multiple_filters(self, mock_list, repository):
        ...
        result = await repository.list(filters={"run_id": "run_456", "risk": "high", "decision": None})

        # Verify the method was called correctly
        mock_list.assert_called_once_with(filters={"run_id": "run_456", "risk": "high", "decision": None})
        assert len(result) == 1
        assert result[0] == mock_approval
