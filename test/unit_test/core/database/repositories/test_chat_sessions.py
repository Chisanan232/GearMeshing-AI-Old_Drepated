...
class TestChatSessionRepository:
    ...
    def sample_chat_message(self):
        ...
        return ChatMessage(session_id=1, role=MessageRole.USER, content="Hello, world!", token_count=15)
    ...
    async def test_list_sessions_with_filters(self, repository, mock_session, sample_chat_session):
        ...
        filters = {"tenant_id": "tenant_456", "agent_role": "developer", "is_active": True, "run_id": "test_run_123"}
    ...
    async def test_get_messages_for_session(self, repository, mock_session):
        ...
        messages = [
            ChatMessage(
                session_id=1, role=MessageRole.USER, content="Hello!", created_at=datetime(2024, 1, 1, 10, 0, 0)
            ),
            ChatMessage(
                session_id=1, role=MessageRole.ASSISTANT, content="Hi there!", created_at=datetime(2024, 1, 1, 10, 1, 0)
            ),
            ChatMessage(
                session_id=1, role=MessageRole.USER, content="How are you?", created_at=datetime(2024, 1, 1, 10, 2, 0)
            ),
        ]
    ...
    async def test_get_messages_for_session_with_limit(self, repository, mock_session):
        ...
        messages = [
            ChatMessage(
                session_id=1, role=MessageRole.USER, content=f"Message {i}", created_at=datetime(2024, 1, 1, 10, i, 0)
            )
            for i in range(10)
        ]
    ...
    async def test_message_chronological_order(self, repository, mock_session):
        ...
        messages = [
            ChatMessage(
                session_id=1, role=MessageRole.USER, content="First message", created_at=datetime(2024, 1, 1, 10, 0, 0)
            ),
            ChatMessage(
                session_id=1,
            ...
            ChatMessage(
                session_id=1, role=MessageRole.USER, content="Third message", created_at=datetime(2024, 1, 1, 10, 2, 0)
            ),
        ]
    ...
    async def test_session_with_multiple_messages(self, repository, mock_session, sample_chat_session):
        ...
        messages = [
            ChatMessage(session_id=1, role=MessageRole.USER, content="Hello!"),
    ...
    async def test_session_business_logic_scenarios(self, repository, mock_session):
        ...
        # Active session
        active_session = ChatSession(title="Active Session", agent_role="developer", is_active=True)

        # Inactive session
        inactive_session = ChatSession(title="Inactive Session", agent_role="developer", is_active=False)
