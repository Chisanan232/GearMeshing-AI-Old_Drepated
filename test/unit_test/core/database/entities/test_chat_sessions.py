...
class TestChatSession:
    ...
    @pytest.mark.parametrize("field_name", ["title", "agent_role"])
...
class TestChatMessage:
    ...
    def test_chat_message_creation_valid_data(self):
        ...
        message = ChatMessage(session_id=1, role=MessageRole.USER, content="Hello, world!")
    ...
    def test_chat_message_automatic_timestamps(self):
        ...
        message = ChatMessage(session_id=1, role=MessageRole.USER, content="Test message")
    ...
    def test_chat_message_role_validation(self):
        ...
        for role in [MessageRole.USER, MessageRole.ASSISTANT, MessageRole.SYSTEM]:
            message = ChatMessage(session_id=1, role=role, content="Test message")
            assert message.role == role

    def test_chat_message_content_validation(self):
        """Test ChatMessage content field validation."""
        # Test non-empty content
        message = ChatMessage(session_id=1, role=MessageRole.USER, content="Non-empty content")
        assert message.content == "Non-empty content"

        # Test empty content (should be allowed)
        empty_message = ChatMessage(session_id=1, role=MessageRole.USER, content="")
    ...
    def test_chat_message_foreign_key_constraint(self):
        ...
        message = ChatMessage(session_id=123, role=MessageRole.USER, content="Test message")
    ...
    def test_chat_message_optional_metadata_fields(self):
        ...
        message_none = ChatMessage(session_id=1, role=MessageRole.USER, content="Simple message")
...
class TestChatSessionIntegration:
    ...
    def test_session_with_multiple_messages(self, sample_chat_session_data):
        ...
        messages = [
            ChatMessage(
                session_id=1, role=MessageRole.USER, content="Hello!"  # Will be set properly after session creation
            ),
            ChatMessage(session_id=1, role=MessageRole.ASSISTANT, content="Hi there!"),
            ChatMessage(session_id=1, role=MessageRole.USER, content="How are you?"),
        ]
    ...
    def test_message_role_distribution(self):
        ...
        roles = [MessageRole.SYSTEM, MessageRole.USER, MessageRole.ASSISTANT, MessageRole.USER, MessageRole.ASSISTANT]

        messages = [ChatMessage(session_id=1, role=role, content=f"Message as {role}") for role in roles]
    ...
    def test_message_content_validation_scenarios(self):
        ...
        test_cases = [
            ...
            (
                "Long message",
                "This is a very long message that contains multiple sentences and should be handled properly by the system without any issues.",
            ),
        ...
        for description, content in test_cases:
            message = ChatMessage(session_id=1, role=MessageRole.USER, content=content)
