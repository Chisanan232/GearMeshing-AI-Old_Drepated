...
class TestAdvancedAIWorkflows:
    ...
    async def test_long_running_workflow(
        ...
        long_running_plan = [
            ...
            {"kind": "action", "capability": "docs_read", "args": {"file_path": "historical_data.csv"}},
    ...
    async def test_workflow_with_retry_mechanism(
        ...
        retry_plan = [
            ...
            {"kind": "action", "capability": "web_search", "args": {"query": "unstable_api_data", "max_results": 10}},
            {
                "kind": "thought",
        ...
        async def mock_web_search(*args, **kwargs):
            ...
            if call_count == 1:
                # First call fails
                return CapabilityResult(ok=False, output={"error": "Network timeout", "retryable": True})
            else:
                # Second call succeeds
                return CapabilityResult(ok=True, output={"results": ["data1", "data2"], "source": "api"})
        ...
        event_calls = cast(MagicMock, engine_deps.events.append).call_args_list
        retry_events = [call for call in event_calls if "retry" in str(call[0][0]).lower()]
    ...
    async def test_workflow_with_dynamic_adaptation(
        ...
        adaptive_plan = [
            ...
            {"kind": "action", "capability": "docs_read", "args": {"file_path": "sample_data.csv"}},
            {
                "kind": "thought",
        ...
        event_calls = cast(MagicMock, engine_deps.events.append).call_args_list
        adaptation_events = [call for call in event_calls if "adapt" in str(call[0][0]).lower()]
    ...
    async def test_workflow_with_resource_constraints(
        ...
        usage_calls = (
            cast(MagicMock, engine_deps.usage.track).call_args_list
            if engine_deps.usage and hasattr(engine_deps.usage, "track")
            else []
        )
    ...
    async def test_workflow_with_external_dependencies(
        ...
        external_plan = [
            ...
            {"kind": "action", "capability": "docs_read", "args": {"file_path": "cached_api_response.json"}},
        ]
        ...
        async def mock_external_call(*args, **kwargs):
            ...
            if call_count == 1:
                ...
                return CapabilityResult(ok=False, output={"error": "Service unavailable", "fallback_available": True})
            else:
                # Fallback succeeds
                return CapabilityResult(ok=True, output={"cached_data": "fallback_results", "timestamp": "2024-01-01"})
