...
def _wait_http_ready(urls: List[str], timeout: float = 30.0) -> str:
    ...
    while time.time() - start < timeout:
        for base in urls:
            # Use sync Direct strategy to probe readiness; it's simpler than spinning up an event loop here
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_clickup_async_direct_strategy_lists_tools() -> None:
