...
from gearmeshing_ai.agent_core.repos.sql import build_sql_repos, SqlRepoBundle
from gearmeshing_ai.agent_core.schemas.domain import AgentRun, AgentEvent, Approval, UsageLedgerEntry, AgentEventType, AgentRunStatus
...
from gearmeshing_ai.server.schemas import (
    SSEEventData, SSEResponse, KeepAliveEvent, ErrorEvent,
    ThinkingData, ThinkingOutputData, OperationData, ToolExecutionData,
    ApprovalRequestData, ApprovalResolutionData, RunStartData,
    RunCompletionData, RunFailureData
)
...
class OrchestratorService:
    ...
    async def submit_approval(self, run_id: str, approval_id: str, decision: str, note: str | None, decided_by: str | None) -> Approval:
    ...
    async def list_usage(self, tenant_id: str, from_date: Optional[datetime] = None, to_date: Optional[datetime] = None) -> List[UsageLedgerEntry]:
        ...
        while True:
            try:
                ...
                if last_event_timestamp is None:
                    ...
                    new_events = [
                        e for e in events
                        if hasattr(e, 'created_at') and e.created_at > last_event_timestamp
                    ]
