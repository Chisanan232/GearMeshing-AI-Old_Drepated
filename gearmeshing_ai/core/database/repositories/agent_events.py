...
class AgentEventRepository(BaseRepository[AgentEvent]):
    ...
    async def get_events_for_run(self, run_id: str, limit: Optional[int] = None) -> List[AgentEvent]:
        ...
        """
        stmt = (
            select(AgentEvent).where(AgentEvent.run_id == run_id).order_by(AgentEvent.created_at.asc())  # type: ignore
        )
    ...
    async def get_events_by_type(self, run_id: str, event_type: str) -> List[AgentEvent]:
        ...
        stmt = (
            select(AgentEvent)
            .where((AgentEvent.run_id == run_id) & (AgentEvent.type == event_type))
            .order_by(AgentEvent.created_at.asc())  # type: ignore
        )
        result = self.session.exec(stmt)
        return list(result)

    async def get_events_by_correlation(self, correlation_id: str) -> List[AgentEvent]:
