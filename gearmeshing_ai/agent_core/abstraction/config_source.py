...
class AgentConfigSource(BaseModel):
    ...
    def get_system_prompt(self) -> str:
        ...
        return prompt_provider.get(
            name=self.prompt_key,
            locale=self.locale,
            tenant=self.prompt_tenant_id
        )
    ...
    def to_agent_config(self, framework: str, name: Optional[str] = None) -> AIAgentConfig:
        ...
        agent_config = AIAgentConfig(
            ...
            temperature=self.overrides.get('temperature') if self.overrides and 'temperature' in self.overrides else model_config.temperature,
            max_tokens=self.overrides.get('max_tokens') if self.overrides and 'max_tokens' in self.overrides else model_config.max_tokens,
